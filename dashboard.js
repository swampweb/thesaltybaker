let tx=[], allCustomers=[];
const accountSets={Income:['Orders','Trade Show'],Expense:['Grocery','Supplies','Equipment','Tax'],Donation:['Donation']};
function setActive(sel,group){document.querySelectorAll(group).forEach(b=>b.classList.toggle('active',b===sel));}
function renderAccountButtons(type,preferred){const wrap=$('accountButtons');const opts=accountSets[type]||accountSets.Income;const chosen=preferred&&opts.includes(preferred)?preferred:opts[0];$('account').value=chosen;wrap.innerHTML=opts.map(a=>`<button type="button" class="choice-btn account-choice ${a===chosen?'active':''}" data-account="${a}">${a}</button>`).join('');wrap.querySelectorAll('.account-choice').forEach(btn=>btn.onclick=()=>{setActive(btn,'.account-choice');$('account').value=btn.dataset.account;});}
function setType(type,preferred){$('entryType').value=type;document.querySelectorAll('[data-type]').forEach(b=>b.classList.toggle('active',b.dataset.type===type));renderAccountButtons(type,preferred);updateLastEntryBubble(type);}
async function fillYears(){const years=await loadYears();const cur=String(new Date().getFullYear());$('year').innerHTML=years.map(y=>`<option ${y===cur?'selected':''}>${y}</option>`).join('');}
...
async function saveEntry(e){e.preventDefault();const row={transaction_date:$('date').value,entry_type:$('entryType').value,account:$('account').value,customer_name:bestCustomerName($('name').value),payment_type:$('paymentType').value,amount:Number($('amount').value)||0,notes:$('notes').value};const id=$('entryId').value;if(id)await updateTransaction(id,row);else await insertTransaction(row);$('entryForm').reset();$('entryId').value='';$('date').value=todayISO();setType('Income','Orders');await fillCustomers();await refresh();}
async function runCustomerReport(){const name=$('customerSearch').value.trim();if(!name)return alert('Type or pick a customer first.');const rows=await sbFetch(`/transactions?select=*&customer_name=eq.${encodeURIComponent(name)}&order=transaction_date.desc,id.desc&limit=5000`);const income=rows.filter(r=>r.entry_type==='Income').reduce((t,r)=>t+Number(r.amount||0),0);const donations=rows.filter(r=>r.entry_type==='Donation').reduce((t,r)=>t+Number(r.amount||0),0);const dates=rows.map(r=>r.transaction_date).sort();$('customerSummary').innerHTML=`<div class="mini-card"><span>Orders / Donations</span><strong>${rows.filter(r=>r.entry_type!=='Expense').length}</strong></div><div class="mini-card"><span>Total Income</span><strong>${money(income+donations)}</strong></div><div class="mini-card"><span>First Date</span><strong>${dates[0]||'-'}</strong></div><div class="mini-card"><span>Last Date</span><strong>${dates[dates.length-1]||'-'}</strong></div>`;$('customerTable').querySelector('tbody').innerHTML=rows.map(r=>`<tr><td>${r.transaction_date}</td><td>${r.entry_type}</td><td>${r.account}</td><td>${r.payment_type||''}</td><td>${money(r.amount)}</td><td>${escapeHtml(r.notes||'')}</td></tr>`).join('')||'<tr><td colspan="6">No history found.</td></tr>';}
function exportCsv(){const header=['Date','Type','Account','Name','Payment','Amount','Notes'];const rows=tx.map(r=>[r.transaction_date,r.entry_type,r.account,r.customer_name,r.payment_type,r.amount,r.notes]);const csv=[header,...rows].map(r=>r.map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(',')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download='salty-baker-visible-transactions.csv';a.click();}
async function init(){ $('nav').innerHTML=nav({key:'dashboard',title:'The Salty Baker Finance',subtitle:'Income, Expenses, Tax Filing, and Customer Tracking'});$('footer').innerHTML=footer();months.forEach((m,i)=>$('month').insertAdjacentHTML('beforeend',`<option value="${i+1}" ${new Date().getMonth()===i?'selected':''}>${m}</option>`));$('date').value=todayISO();$('paymentType').innerHTML=['','Apple','Cash','Cash App','Paypal','Square','Venmo','Zelle','Other'].map(x=>`<option value="${x}">${x}</option>`).join('');document.querySelectorAll('[data-type]').forEach(btn=>btn.onclick=()=>setType(btn.dataset.type));setType('Income','Orders');await fillYears();await fillCustomers();await refresh();$('entryForm').onsubmit=saveEntry;$('resetBtn').onclick=()=>{$('entryForm').reset();$('entryId').value='';$('date').value=todayISO();setType('Income','Orders');};$('year').onchange=refresh;$('month').onchange=refresh;$('search').oninput=renderTx;$('name').oninput=e=>fillCustomers(e.target.value);$('customerSearch').oninput=e=>fillCustomers(e.target.value);$('runCustomerReport').onclick=runCustomerReport;$('exportCsv').onclick=exportCsv;}
